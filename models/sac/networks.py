...
from utils.misc import CategoricalMasked, first_not_none
...
class ActorNetwork(BaseNetwork):
    ...
    def __init__(self, num_observation:int, num_action:int, hidden_layers:Optional[List[int]]=[32,64], activation:Optional[str] = "ReLU"):
    ...
    def evaluate(self, state, epsilon=1e-6, mask: Optional[np.ndarray] = None) -> Tuple[List[float], torch.Tensor, torch.Tensor]:
        ...
        # action_probs[~mask_tensor.any(dim=-1),:] = 0.0
    ...
    def get_det_action(self, state_obs: torch.Tensor, mask: Optional[np.ndarray] = None)->List[Optional[Union[int, float]]]:
        with torch.no_grad():
...
class CriticNetwork(BaseNetwork):
    ...
    def __init__(self, num_observation, num_action, hidden_layers:Optional[List[int]]=[32,64], activation:Optional[str] = "ReLU", seed:int=1):
        """Initialize parameters and build model.
        Params
        ======
            state_size (int): Dimension of each state
