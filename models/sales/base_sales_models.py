...
#                  load_from: Optional[Union[Path, str, dict]] = None, name: Optional[str] = None,
#                  info: Optional[Dict[str, Any]] = None, device: Optional[torch.device] = None, **kwargs):
#         super().__init__()
#
#         self._device = device if device is not None else torch.device("cpu")
#         self._model_state_dict_temp: Optional[Dict[str, Any]] = None
#
#         # Load model if load_path is provided
#         if load_from is not None:
#             self.load(load_from=load_from)
#
...
#                  hidden_layers: Sequence[int] = (256, 512, 1024, 512, 256),  # (64, 256, 64),
#                  activation: Optional[Activations] = "ReLU", noise_std: float = 2.0,
#                  load_from: Optional[Union[Path, str, dict]] = None,
...
#
#         self._repr = f"{self.name}(input_dim={self._input_dim}, " \
#                      f"output_dim={self._output_dim}, " \
#                      f"hidden_layers={self._hidden_layers}, " \
#                      f"activation={getattr(nn, self._activation)().__class__.__name__}, " \
...
#             self._dynamic_std = self._noise_std
#             self._step_count = 0
#
#         if load_from is not None:
#             self._load_model_state_dict()
#
#     def _initiate_model(self, *args, **kwargs):
#         layers = []
#         in_dim = self._input_dim
#         for layer_dim in self._hidden_layers:
#             layers += [nn.LayerNorm(in_dim)]
#             layers += [nn.Linear(in_features=in_dim, out_features=layer_dim)]
#             in_dim = layer_dim
#             if self._activation is not None:
#                 layers += [getattr(nn, self._activation)()]
#         layers += [nn.Linear(in_features=in_dim, out_features=self._output_dim)]
...
#     def get_sales(self, inputs: torch.Tensor) -> torch.Tensor:
#         with torch.no_grad():
#             sales = ((self.forward(inputs).exp() - 1) * self._sales_normalising_factor).cpu()
#         return sales
#
#
...
#                 pred = self._models[flavour](input_row).detach().cpu()
...
#             for flavour, input_row in zip(flavours, inputs_without_encoding):
#                 # Get the prediction from the corresponding model
#                 single_sales = self._models[flavour].get_sales(input_row).detach().cpu()
#                 if "sales" not in locals():
#                     sales = single_sales
#                 else:
#                     sales = torch.vstack([sales, single_sales])
#         return sales
    ...
    def __init__(self, load_from_dir: Optional[Union[Path, str]] = None,
        ...
        if load_from_dir is not None:
            ...
            self._repr = f"{self.name}(input_dim={self._input_dim}, output_dim={self._output_dim}, " \
                         ...
                         f"models={[f'{flavour}: {model}' for flavour, model in self._models.items()]})"
        else:
            self.load(load_from_file)
